    ...
    switch(arguments.length) {
        ...
        case 3:
            ...
            Object.defineProperty(target, props, {
                ...
                set: isArray ? (value) => (<ObservableArray<any>><any>observable).replace(value) : observable,
                enumerable: true,
                configurable: true
            });
            break;
...
mobservableStatic.turnObservablesIntoProperties = function turnObservablesIntoProperties(object:Object) {
    for(var key in object) if (object.hasOwnProperty(key)) {
        var observable = object[key], 
            getter:()=>any = null,
            setter:(v)=>void = null;
        if (observable instanceof ObservableArray) {
            getter = function() { return observable; };
            setter = (newValue) => (<ObservableArray<any>>observable).replace(newValue);
        } else if (observable && observable.prop && (observable.prop instanceof ObservableValue)) {
...
class ComputedObservable<U> extends ObservableValue<U> {
    ...
    constructor(protected func:()=>U, private scope?:Object) {
        ...
        this.dependencyState.nextState = () => this.compute();
    }
