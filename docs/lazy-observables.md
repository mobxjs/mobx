...
Functions `onBecomeObserved` and `onBecomeUnobserved` can be used to attach lazy behavior or side effects to existing observables. They are hooked into the observability system of MobX and get notified when the observables _start_ and _stop_ becoming observed. They both return a _disposer_ function that detaches the _listener_.
