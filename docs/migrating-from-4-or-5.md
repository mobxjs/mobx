...
5. For TypeScript users:
    ...
    - If you want to use decorators, use TypeScript 5+ Stage 3 decorators and make sure `experimentalDecorators` is disabled. Legacy decorators are not supported in MobX 7. Check out the [Enabling decorators {🚀}](enabling-decorators.md) section for more details.
...
Fields can be made observable with either explicit annotation maps or Stage 3 decorators:
...
To get rid of all uses of legacy MobX decorators and replace them with the equivalent `makeObservable` calls, go to the directory that contains your source code and run:

```shell
npx mobx-undecorate
```

If you want to retain decorators, migrate them to Stage 3 decorators manually as described in [Enabling decorators {🚀}](enabling-decorators.md).
