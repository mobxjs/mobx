...
const packageName = "mobx"
const input = "src/mobx.ts"
...
if (process.env.NODE_ENV === 'production') {
  module.exports = require('./${packageName}.cjs.production.min.js')
} else {
...
const copyMtsTypes = () => ({
    name: "copy-mts-types",
    closeBundle() {
        const dts = path.join(dist, `${packageName}.d.ts`)
        const dmts = path.join(dist, `${packageName}.d.mts`)
...
const createConfig = ({ format, env, declarations, extraOutputs = [] }) => {
    ...
    const outputName = [`${dist}/${packageName}`, format, env, shouldMinify ? "min" : "", "js"]
        .filter(Boolean)
        .join(".")
    ...
    return {
        ...
        plugins: [
            ...
            shouldMinify && terser(mobxTerserOptions),
