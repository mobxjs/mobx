...
function interpret_cli_args() {
    ...
    let arg_without_equal = USER_ARGS.slice()
        .filter(v => !v.includes("="))
        .findIndex(kwarg => kwarg.includes("--dir") || kwarg.includes("--path"))
    var arg_with_equal = USER_ARGS.slice().find(v => v.includes("--dir=") || v.includes("--path="))

    //use cwd as default, but will override it with user args if they exist for --dir or --path
    var PARSED_INPUT = ""

    let is_arg_directory_only =
        process.argv[2] && fs.existsSync(path.resolve(process.cwd(), process.argv[2] || ""))
...
spawnBin("jscodeshift", [
    ...
    "-t",
    `"${path.join(__dirname, "src", "undecorate.ts")}"`,
